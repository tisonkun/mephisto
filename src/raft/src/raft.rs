// Copyright 2023 CratesLand Contributors
//
...
use std::{
    assert, assert_eq, assert_ne, cmp, debug_assert_eq,
    ops::{Deref, DerefMut},
    panic,
};
...
use crate::{
    confchange,
    confchange::Changer,
    eraftpb::{ConfChange, ConfState, Entry, EntryType, HardState, Message, MessageType, Snapshot},
    ...
    util,
    util::NO_LIMIT,
    Config, Progress, ProgressState, ProgressTracker,
};
...
pub struct RaftCore<T: Storage> {
    ...
    /// value.
    ///
    /// This value is conservatively set in cases where there may be a configuration change
    /// pending, but scanning the log is possibly expensive. This implies that the index stated
    /// here may not necessarily be a config change entry, and it may not be a
    /// `BeginMembershipChange` entry, even if we set this to one.
...
impl<T: Storage> Raft<T> {
    ...
    pub fn new(c: &Config, store: T) -> Result<Self> {
        c.validate()?;
        ...
        if !crate::proto::conf_state_eq(&new_cs, conf_state) {
            panic!("invalid restore: {:?} != {:?}", conf_state, new_cs);
        }
...
impl<T: Storage> RaftCore<T> {
    ...
    fn send(&mut self, mut m: Message, msgs: &mut Vec<Message>) {
            ...
            || m.msg_type() == MessageType::MsgRequestPreVoteResponse
        {
            if m.term == 0 {
                ...
                // sending.
                // - MsgVote: m.Term is the term the node is campaigning for, non-zero as we
                //   increment the term when campaigning.
                // - MsgVoteResp: m.Term is the new r.Term if the MsgVote was granted, non-zero for
                //   the same reason MsgVote is
                // - MsgPreVote: m.Term is the term the node will campaign, non-zero as we use
                //   m.Term to indicate the next term we'll be campaigning for
                // - MsgPreVoteResp: m.Term is the term received in the original MsgPreVote if the
                //   pre-vote was granted, non-zero for the same reasons MsgPreVote is
                panic!("term should be set when sending {:?}", m.msg_type());
            }
        } else {
            if m.term != 0 {
                panic!(
    ...
    fn prepare_send_snapshot(&mut self, m: &mut Message, pr: &mut Progress, to: u64) -> bool {
        ...
        debug!(
            progress = ?pr,
            "[first_index: {first_index}, commit: {committed}] sent snapshot[index: {snapshot_index}, term: {snapshot_term}] to {to}",
    ...
    fn maybe_send_append(
        ...
        if pr.is_paused() {
            trace!(
                progress = ?pr,
                "Skipping sending to {}, it's paused",
                to
            );
            return false;
        }
    ...
    fn send_heartbeat(
        ...
        let m = Message {
            ...
            commit: cmp::min(pr.matched, self.raft_log.committed),
...
impl<T: Storage> Raft<T> {
    ...
    pub fn commit_apply(&mut self, applied: u64) {
            ...
            && self.state == StateRole::Leader
        {
            ...
            // nil Data which unmarshall into an empty ConfChange and has the
            ...
            info!(config = ?self.prs.conf(), "initiating automatic transition out of joint configuration");
        }
    }
    ...
    pub fn become_follower(&mut self, term: u64, leader_id: u64) {
        ...
        info!("became follower at term {}", self.term);
    }
    ...
    pub fn become_pre_candidate(&mut self) {
        ...
        self.leader_id = INVALID_ID;
        info!("became pre-candidate at term {}", self.term);
    }
        ...
        if m.term == 0 {
                ...
                || m.msg_type() == MessageType::MsgRequestPreVote
            {
                ...
                if !force && in_lease {
                    ...
                    info!(
                        ...
                        msg_type = ?m.msg_type(),
                        "[log_term: {}, index: {}, vote: {}] ignored vote from {} [log_term: {}, index: {}]: lease is not expired",
                ...
                || (m.msg_type() == MessageType::MsgRequestPreVoteResponse && !m.reject)
            {
                ...
                info!(
                    ...
                    message_term = m.term,
                    msg_type = ?m.msg_type(),
                    "received a message with higher term from {}",
                    m.from,
                );
                    ...
                    || m.msg_type() == MessageType::MsgAppend)
            {
                ...
                info!(
                    ...
                    msg_type = ?m.msg_type(),
                    msg_term = m.term,
                    "ignored a message with lower term from {}",
                    m.from,
                );
            }
            return Ok(());
    ...
    fn handle_append_response(&mut self, m: &Message) {
        ...
        if m.reject && m.log_term > 0 {
            ...
            //   term (F)   1
            //
            // 3. term 3 leader goes down, term 2 leader returns as term 4 leader. It commits the
            //    log & entries at term 4.
            //
            //              -----------------
            //   term (L)   1 2 2 2
            ...
            //
            // 4. the leader will now probe the returning follower at index 7, the rejection points
            //    it at the end of the follower's log which is at a higher log term than the
        ...
        let pr = match self.prs.get_mut(m.from) {
            Some(pr) => pr,
            None => {
                debug!("no progress available for {}", m.from);
                return;
            }
        };
        ...
        match pr.state {
            ...
            ProgressState::Snapshot => {
                if pr.is_snapshot_caught_up() {
                    debug!(
                        progress = ?pr,
                        "snapshot caught up, resumed sending replication messages to {}",
                        m.from
                    );
                    pr.become_probe();
                }
        ...
        if Some(m.from) == self.r.lead_transferee {
            ...
            if pr.matched == last_index {
                info!("sent MsgTimeoutNow to {} after received MsgAppResp", m.from);
    ...
    fn handle_heartbeat_response(&mut self, m: &Message) {
        ...
        for rs in self.r.read_only.advance(&m.context) {
    ...
    fn handle_transfer_leader(&mut self, m: &Message) {
        ...
        if self.prs.conf().learners.contains(&from) {
            debug!(to = from, "ignored transferring leadership");
            return;
        }
        ...
        if let Some(last_lead_transferee) = self.lead_transferee {
            ...
            info!(
                ...
                self.term, last_lead_transferee,
            );
        }
            ...
            MessageType::MsgPropose => {
                ...
                for (i, e) in m.entries.iter_mut().enumerate() {
                    ...
                    if e.entry_type() == EntryType::EntryConfChange {
                        ...
                        if let Err(err) = cc.merge(e.data.as_slice()) {
                            error!(err = ?err, "invalid confchange");
                            return Err(Error::ProposalDropped);
                        }
                    } else {
                        continue;
                ...
                if !self.append_entry(&mut m.entries) {
                    ...
                    debug!(
                        "entries are dropped due to over limit of max uncommitted size, uncommitted_size: {}",
    ...
    fn maybe_commit_by_vote(&mut self, m: &Message) {
        ...
        info!("[commit: {}, last_index: {}, last_term: {}] fast-forwarded commit to vote request [index: {}, term: {}]",
    ...
    fn step_candidate(&mut self, m: Message) -> Result<()> {
        match m.msg_type() {
            MessageType::MsgPropose => {
                info!("no leader at term {}; dropping proposal", self.term);
                return Err(Error::ProposalDropped);
            }
            ...
            MessageType::MsgTimeoutNow => {
                debug!(state = ?self.state, "{} ignored MsgTimeoutNow from {}", self.term, m.from)
            }
            _ => {}
        }
        Ok(())
    ...
    fn step_follower(&mut self, mut m: Message) -> Result<()> {
        match m.msg_type() {
            ...
            MessageType::MsgReadIndexResp => {
                if m.entries.len() != 1 {
                    error!(
                        entries_count = m.entries.len(),
                        "invalid format of MsgReadIndexResp from {}", m.from
                    );
                    return Ok(());
                }
                let rs = ReadState {
                    index: m.index,
                    request_ctx: std::mem::take(&mut m.entries).remove(0).data,
                };
                self.read_states.push(rs);
                // `index` and `term` in MsgReadIndexResp is the leader's commit index and its
                // current term, the log entry in the leader's commit index will
                // always have the leader's current term, because the leader only
                // handle MsgReadIndex after it has committed log entry in its term.
    ...
    pub fn request_snapshot(&mut self) -> Result<()> {
        if self.state == StateRole::Leader {
            ...
            info!(term = self.term, "no leader; dropping request snapshot");
    ...
    pub fn handle_append_entries(&mut self, m: &Message) {
            ...
            .maybe_append(m.index, m.log_term, m.commit, &m.entries)
        {
            ...
            debug!(
                index = m.index,
                log_term = ?self.raft_log.term(m.index),
                "rejected msgApp [log_term: {}, index: {}] from {}",
                m.log_term, m.index, m.from,
            );
    ...
    fn handle_snapshot(&mut self, mut m: Message) {
        ...
        if self.restore(m.snapshot.take().unwrap_or_default()) {
            info!(
                "[commit: {}, term: {}] restored snapshot [index: {}, term: {}]",
                self.term, self.raft_log.committed, sindex, sterm
            );
            let to_send = Message {
                msg_type: MessageType::MsgAppendResponse as i32,
                to: m.from,
            ...
            info!(
                ...
                self.raft_log.committed, sindex, sterm
            );
            let to_send = Message {
                msg_type: MessageType::MsgAppendResponse as i32,
                to: m.from,
    ...
    pub fn restore(&mut self, snap: Snapshot) -> bool {
        ...
        if self.state != StateRole::Follower {
            ...
            warn!(state = ?self.state, "non-follower attempted to restore snapshot");
        ...
        if cs
            ...
            .all(|id| *id != self.id)
        {
            warn!(conf_state = ?cs, "attempted to restore snapshot but it is not in the ConfState");
            return false;
        }
        ...
        if let Err(e) = confchange::restore(&mut self.prs, last_index, cs) {
            ...
            panic!("unable to restore config {:?}: {}", cs, e);
        }
        ...
        let cs = self
            ...
            .get_metadata()
            .get_conf_state();
        if !crate::proto::conf_state_eq(cs, &new_cs) {
            panic!("invalid restore: {:?} != {:?}", cs, new_cs);
        }
    ...
    pub fn post_conf_change(&mut self) -> ConfState {
        info!(config = ?self.prs.conf(), "switched to configuration");
        ...
        if let Some(ctx) = self.read_only.last_pending_request_ctx() {
            ...
            {
                for rs in self.r.read_only.advance(&ctx) {
                    if let Some(m) = self.handle_ready_read_index(rs.req, rs.index) {
                        self.r.send(m, &mut self.msgs);
                    }
                }
    ...
    pub fn reset_randomized_election_timeout(&mut self) {
        ...
        debug!(
            ...
            prev_timeout, timeout, self.election_elapsed
        );
    ...
    fn handle_ready_read_index(&mut self, mut req: Message, index: u64) -> Option<Message> {
        if req.from == INVALID_ID || req.from == self.id {
            let rs = ReadState {
                index,
                request_ctx: std::mem::take(&mut req.entries).remove(0).data,
            };
            self.read_states.push(rs);
            return None;
        }
