...
use crate::{
    eraftpb::{ConfChangeSingle, ConfChangeType},
    tracker::{Configuration, ProgressMap, ProgressTracker},
    Error, Result,
};
...
impl Changer<'_> {
    ...
    /// Copies the tracker's config. It returns an error if `check_invariants` does.
...
fn check_invariants(cfg: &Configuration, prs: &IncrChangeMap) -> Result<()> {
    // Intentionally allow the empty config. In production we'll never see a
