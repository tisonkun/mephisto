...
use crate::{
    ...
    errors::{Error, Result},
    read_only::ReadState,
    storage::GetEntriesFor,
    GetEntriesContext, Raft, SoftState, StateRole, Status, Storage,
};
...
impl Display for Peer {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "n{}({})", self.id, self.address)
    }
}
...
impl<T: Storage> RawNode<T> {
    ...
    pub fn new(config: &Config, store: T) -> Result<Self> {
        ...
        info!("RawNode created with id {id}.", id = rn.raft.id);
        Ok(rn)
    }
    ...
    pub fn propose_conf_change(&mut self, context: Vec<u8>, cc: ConfChange) -> Result<()> {
        let (data, ty) = (cc.encode_to_vec(), EntryType::EntryConfChange);
        let m = Message {
            msg_type: MessageType::MsgPropose as i32,
            entries: vec![Entry {
                entry_type: ty as i32,
                data,
                context,
                ..Default::default()
            }],
    ...
    /// [`LightReady`] contains updates that only valid after persisting last ready. It should
    /// also be fully processed. Then [`Self::advance_apply`] or [`Self::advance_apply_to`]
    ...
    pub fn advance_append(&mut self, rd: Ready) -> LightReady {
        ...
        if hard_state.commit > self.prev_hs.commit {
            ...
            assert_eq!(hard_state.commit, self.prev_hs.commit);
    ...
    ///
    /// Raft works on an assumption persisted updates should not be lost, which usually requires
    /// expensive operations like `fsync`. `advance_append_async` allows you to control the rate
    /// of such operations and get a reasonable batch size. However, it's still required that
    /// the updates can be read by raft from the `Storage` trait before calling
