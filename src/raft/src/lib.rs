...
//!
//! ```rust
//! use mephisto_raft::{raw_node::RawNode, storage::MemStorage, Config};
//!
...
//!
//! ```rust
//! # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode};
...
//! write flow is asynchronous in Raft, so the write log entry must be replicated to other
//! followers, then committed and at last applied to the state machine, so here we need a way to
//! notify the client after the write is finished.
//!
...
//!
//! ```rust
//! # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode, eraftpb::Message};
//! # use std::{
//! #     sync::mpsc::{channel, RecvTimeoutError},
...
//! propose the request ID to the Raft log. In your own practice, you can embed the ID in your
//! request and propose the encoded binary request data.
//!
//! ## Processing the `Ready` State
//!
//! When your Raft node is ticked and running, Raft should enter a `Ready` state. You need to first
//! use `has_ready` to check whether Raft is ready. If yes, use the `ready` function to get a
//! `Ready` state:
//!
//! ```rust
//! # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode};
...
//! ```
//!
//! The `Ready` state contains quite a bit of information, and you need to check and process them
//! one by one:
//!
//! 1. Check whether `messages` is empty or not. If not, it means that the node will send messages
//! to other nodes:
//!
//!     ```rust
//!     # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode, StateRole};
//!     #
//!     # let config = Config { id: 1, ..Default::default() };
//!     # config.validate().unwrap();
//!     # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
//!     ```
//!
//! 2. Check whether `snapshot` is empty or not. If not empty, it means that the Raft node has
//! received a Raft snapshot from the leader and we must apply the snapshot:
//!
//!     ```rust
//!     # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode};
//!     #
...
//!
//!     ```rust
//!     # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode, eraftpb};
//!     #
//!     # let config = Config { id: 1, ..Default::default() };
//!     # config.validate().unwrap();
//!     # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
//!     # let mut ready = node.ready();
//!     #
//!     # fn handle_conf_change(e: eraftpb::Entry) {
//!     # }
//!     #
//!     # fn handle_normal(e: eraftpb::Entry) {
//!     # }
//!     #
...
//!             eraftpb::EntryType::EntryNormal => handle_normal(entry),
//!             eraftpb::EntryType::EntryConfChange => handle_conf_change(entry),
//!         }
//!     }
//!     ```
//!
//!     Note, although Raft guarantees only persisted committed entries will be applied,
//!     but it doesn't guarantee commit index is persisted before being applied. For example,
...
//!     ```
//!
//! 5. Check whether `hs` is empty or not. If not empty, it means that the `HardState` of the node
//! has changed. For example, the node may vote for a new leader, or the commit index has been
//! increased. We must persist the changed `HardState`:
//!
//!     ```rust
//!     # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode};
//!     #
...
//!         node.mut_store().wl().set_hard_state(hs.clone());
//!     }
//!     ```
//!
//! 6. Check whether `persisted_messages` is empty or not. If not, it means that the node will send
//! messages to other nodes after persisting hardstate, entries and snapshot:
//!
//!     ```rust
//!     # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode, StateRole};
//!     #
...
//!     ```
//!
//! 7. Call `advance` to notify that the previous work is completed. Get the return value
//! `LightReady` and handle its `messages` and `committed_entries` like step 1 and step 3 does. Then
//! call `advance_apply` to advance the applied index inside.
//!
//!     ```rust
//!     # use mephisto_raft::{Config, storage::MemStorage, raw_node::RawNode};
//!     # use mephisto_raft::eraftpb::{EntryType, Entry, Message};
//!     #
//!     # let config = Config { id: 1, ..Default::default() };
//!     # config.validate().unwrap();
//!     # let store = MemStorage::new_with_conf_state((vec![1], vec![]));
...
//! above except:
//! 1. All writes are not required to be persisted immediately, they can be written into memory
//! caches; 2. Persisted messages should be sent after all corresponding writes are persisted;
...
//!
//! When building a resilient, scalable distributed system there is a strong need to be able to
//! change the membership of a peer group *dynamically, without downtime.* This Raft crate supports
//! this via **Joint Consensus**
//! ([Raft paper, section 6](https://web.stanford.edu/~ouster/cgi-bin/papers/raft-atc14)).
//!
//! It permits resilient arbitrary dynamic membership changes. A membership change can do any or all
//! of the following:
//!
...
//! ```no_run
//! # use mephisto_raft::{Config, eraftpb::*, proto, raw_node::RawNode, storage::MemStorage};
//! #
...
//! let steps = vec![
//!     proto::new_conf_change_single(4, ConfChangeType::AddNode),
//!     proto::new_conf_change_single(3, ConfChangeType::RemoveNode),
//! ];
...
//!
//! > **Note:** In order to maintain resiliency guarantees  (progress while a majority of both peer
//! > sets is
...
pub use proto::eraftpb;
pub use quorum::{joint::Configuration as JointConfig, majority::Configuration as MajorityConfig};
...
pub mod prelude {
    ...
    pub use crate::{
        config::Config,
        proto::eraftpb::{
            ConfChange, ConfChangeSingle, ConfChangeTransition, ConfChangeType, ConfState, Entry,
            EntryType, HardState, Message, MessageType, Snapshot, SnapshotMetadata,
        },
        raft::Raft,
        raw_node::{Peer, RawNode, Ready, SnapshotStatus},
        read_only::{ReadOnlyOption, ReadState},
        status::Status,
