...
pub struct RaftState {
    /// Contains the last meta information including commit index, the vote leader, and the vote
    /// term.
...
pub trait Storage {
    ...
    /// found in the range.
    ///
    /// Entries are supported to be fetched asynchronously depending on the context. Async is
    /// optional. Storage should check context.can_async() first and decide whether to fetch
    /// entries asynchronously based on its own implementation. If the entries are fetched
    /// asynchronously, storage should return LogTemporarilyUnavailable, and application needs
    /// to call `on_entries_fetched(context)` to trigger re-fetch of the entries after the
    /// storage finishes fetching the entries.
    ///
    /// # Panics
    ///
...
impl MemStorageCore {
    ...
    pub fn apply_snapshot(&mut self, snapshot: Snapshot) -> Result<()> {
        let meta = snapshot.metadata.unwrap_or_default();
...
mod test {
    ...
    use crate::{
        eraftpb::{ConfState, Entry, Snapshot, SnapshotMetadata},
        errors::{Error as RaftError, StorageError},
    };
