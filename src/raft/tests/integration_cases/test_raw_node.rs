...
fn new_raw_node(
    ...
    new_raw_node_with_config(peers, &config, storage)
}
...
fn test_raw_node_step() {
    for msg_t in enum_iterator::all() {
        let s = new_storage();
        s.wl().set_hard_state(hard_state(1, 1, 0));
...
fn test_raw_node_propose_and_conf_change() {
    let mut test_cases = vec![(
        conf_change_with_single(ConfChangeType::AddNode, 2),
        conf_state_simple(vec![1, 2], vec![]),
        None,
    )];

    // Proposing the same as a change works just the same, without entering a joint config.
    let single = new_conf_change_single(2, ConfChangeType::AddNode);
    test_cases.push((
        conf_change(vec![single]),
        conf_state_simple(vec![1, 2], vec![]),
        None,
    ));
    ...
    test_cases.push((
        conf_change(vec![single]),
        conf_state_simple(vec![1], vec![2]),
        None,
    ));

    // We can ask explicitly for joint consensus if we want it.
    let single = new_conf_change_single(2, ConfChangeType::AddLearnerNode);
    let mut cc = conf_change(vec![single]);
    cc.set_transition(ConfChangeTransition::Explicit);
    let cs = conf_state(vec![1], vec![2], vec![1], vec![], false);
    test_cases.push((cc, cs, Some(conf_state_simple(vec![1], vec![2]))));

    // Ditto, but with implicit transition (the harness checks this).
    let single = new_conf_change_single(2, ConfChangeType::AddLearnerNode);
    let mut cc = conf_change(vec![single]);
    cc.set_transition(ConfChangeTransition::Implicit);
    let cs = conf_state(vec![1], vec![2], vec![1], vec![], true);
    test_cases.push((cc, cs, Some(conf_state_simple(vec![1], vec![2]))));
    ...
    let cs = conf_state(vec![2], vec![3], vec![1], vec![1], true);
    test_cases.push((cc, cs, Some(conf_state_simple(vec![2], vec![1, 3]))));

    // Ditto explicit.
    let mut cc = conf_change(vec![
        new_conf_change_single(2, ConfChangeType::AddNode),
        new_conf_change_single(1, ConfChangeType::AddLearnerNode),
        new_conf_change_single(3, ConfChangeType::AddLearnerNode),
    ]);
    cc.set_transition(ConfChangeTransition::Explicit);
    let cs = conf_state(vec![2], vec![3], vec![1], vec![1], false);
    test_cases.push((cc, cs, Some(conf_state_simple(vec![2], vec![1, 3]))));
    ...
    for (cc, exp, exp2) in test_cases {
        ...
        while cs.is_none() {
                ...
                |rn: &mut RawNode<MemStorage>, committed_entries: Vec<Entry>| {
                    for e in committed_entries {
                        if e.entry_type() == EntryType::EntryConfChange {
                            ...
                            cc.merge(e.data.as_slice()).unwrap();
            ...
            if !proposed && is_leader {
                ...
                ccdata = cc.encode_to_vec();
                raw_node.propose_conf_change(vec![], cc.clone()).unwrap();
                proposed = true;
            }
        }
        ...
        assert_eq!(entries[0].data.as_slice(), b"somedata");
        assert_eq!(entries[1].entry_type(), EntryType::EntryConfChange);
        assert_eq!(ccdata, entries[1].data.as_slice());
        assert_eq!(exp, cs.unwrap());

        let conf_index = if cc.enter_joint() == Some(true) {
        ...
        assert_eq!(rd.entries()[0].entry_type(), EntryType::EntryConfChange);
        let mut leave_cc = ConfChange::default();
        leave_cc.merge(rd.entries()[0].data.as_slice()).unwrap();
        assert_eq!(context, leave_cc.context.as_slice(), "{:?}", cc);
...
fn test_raw_node_joint_auto_leave() {
    let single = new_conf_change_single(2, ConfChangeType::AddLearnerNode);
    let mut test_cc = conf_change(vec![single]);
    test_cc.set_transition(ConfChangeTransition::Implicit);
    let exp_cs = conf_state(vec![1], vec![2], vec![1], vec![], true);
    let exp_cs2 = conf_state_simple(vec![1], vec![2]);
...
fn test_raw_node_propose_add_duplicate_node() {
            ...
            |rn: &mut RawNode<MemStorage>, committed_entries: Vec<Entry>| {
                for e in committed_entries {
                    if e.entry_type() == EntryType::EntryConfChange {
                        ...
                        conf_change.merge(e.data.as_slice()).unwrap();
    ...
    let cc1 = conf_change_with_single(ConfChangeType::AddNode, 1);
    ...
    let cc2 = conf_change_with_single(ConfChangeType::AddNode, 2);
    ...
    assert_eq!(entries[0].data.as_slice(), ccdata1.as_slice());
    assert_eq!(entries[2].data.as_slice(), ccdata2.as_slice());
}
...
fn test_raw_node_propose_add_learner_node() -> Result<()> {
    ...
    let cc = conf_change_with_single(ConfChangeType::AddLearnerNode, 2);
    raw_node.propose_conf_change(vec![], cc).expect("");
...
fn test_raw_node_restart() {
    ...
    let mut raw_node = {
        let store = new_storage();
        store.wl().set_hard_state(hard_state(1, 1, 0));
...
fn test_raw_node_restart_from_snapshot() {
    ...
    let mut raw_node = {
        ...
        store.wl().append(&entries).unwrap();
        store.wl().set_hard_state(hard_state(1, 3, 0));
...
fn test_bounded_uncommitted_entries_growth_with_partition() {
    ...
    let s = new_storage();
    let mut raw_node = new_raw_node_with_config(vec![1], config, s.clone());

    // wait raw_node to be leader
    raw_node.campaign().unwrap();
    loop {
        let rd = raw_node.ready();
        s.wl().set_hard_state(rd.hs().unwrap().clone());
        s.wl().append(rd.entries()).unwrap();
        if rd
            .ss()
...
fn test_raw_node_with_async_entries() {
    ...
    let s = new_storage();
    let mut raw_node = new_raw_node_with_config(vec![1, 2], &cfg, s.clone());
...
fn test_raw_node_with_async_entries_on_follower() {
    ...
    let msg = Message {
        to: 1,
        msg_type: MessageType::MsgCheckQuorum as i32,
        ..Default::default()
    };
...
fn test_raw_node_entries_after_snapshot() {
    ...
    let mut raw_node = new_raw_node(1, vec![1, 2], 10, 1, s.clone());
...
fn test_raw_node_overwrite_entries() {
    ...
    let mut raw_node = new_raw_node(1, vec![1, 2, 3], 10, 1, s.clone());
...
fn test_committed_entries_pagination() {
    let s = new_storage();
    let mut raw_node = new_raw_node(1, vec![1, 2, 3], 10, 1, s);
...
/// - `next_entries` returns entries [2..11) in committed_entries (but index 10 already exceeds
///   maxBytes), which isn't noticed internally by Raft
...
/// - Raft does not emit a HardState, but when the app calls advance(), it bumps its internal
///   applied index cursor to 10 (when it should be 9)
...
fn test_committed_entries_pagination_after_restart() {
    ...
    let mut raw_node = RawNode::new(&config, s).unwrap();
