...
use crate::{harness::*, integration_cases::test_raft_paper::commit_noop_entry};
...
fn test_progress_committed_index() {
    ...
    let test_entries = Entry {
        data: (b"testdata" as &'static [u8]).into(),
        ..Default::default()
    };
    ...
    // [msg_type: MsgAppend to: 1 from: 2 term: 2 log_term: 2 index: 6 entries {term: 2 index: 7
    // data: "somedata"} commit: 4, msg_type: MsgAppend to: 3 from: 2 term: 2 log_term: 2 index:
    // 6 entries {term: 2 index: 7 data: "somedata"} commit: 4]
    let msg_append = nt.read_messages();

    nt.dispatch(msg_append).expect("");

    // [msg_type: MsgAppendResponse to: 2 from: 1 term: 2 index: 6 commit: 4 reject: true
    // reject_hint: 4, msg_type: MsgAppendResponse to: 2 from: 3 term: 2 index: 6 commit: 4
    // reject: true reject_hint: 4]
    let msg_append_response = nt.read_messages();

    nt.dispatch(msg_append_response).expect("");

    // [msg_type: MsgAppend to: 3 from: 2 term: 2 log_term: 2 index: 4 entries {term: 2 index: 5
    // data: "somedata"} entries {term: 2 index: 6 data: "somedata"} entries {term: 2 index: 7 data:
    // "somedata"} commit: 4, msg_type: MsgAppend to: 1 from: 2 term: 2 log_term: 2 index: 4
    // entries {term: 2 index: 5 data: "somedata"} entries {term: 2 index: 6 data: "somedata"}
    // entries {term: 2 index: 7 data: "somedata"} commit: 4]
    let msg_append = nt.read_messages();
...
fn test_progress_paused() {
    ...
    let m = Message {
        ...
        entries: vec![Entry {
            data: (b"some_data" as &'static [u8]).into(),
            ..Default::default()
        }],
        ..Default::default()
    };
...
fn test_leader_election_overwrite_newer_logs_with_config(pre_vote: bool) {
    ...
    // - Node 1 replicated a log entry to node 2 but died before sending it to other nodes.
    // - Node 3 won the second election in term 2.
    // - Node 3 wrote an entry to its logs but died without sending it to any other nodes.
    //
    ...
    let mut network = Network::new_with_config(
        vec![
            Some(ents_with_config(&[1], pre_vote, 1, peers.clone())), /* Node 1: Won first
                                                                       * election */
            Some(ents_with_config(&[1], pre_vote, 2, peers.clone())), /* Node 2: Get logs
                                                                       * from node 1 */
            Some(ents_with_config(&[2], pre_vote, 3, peers.clone())), /* Node 3: Won second
                                                                       * election */
            Some(voted_with_config(3, 2, pre_vote, 4, peers.clone())), /* Node 4: Voted but
                                                                        * didn't get logs */
            Some(voted_with_config(3, 2, pre_vote, 5, peers)), /* Node 5: Voted but didn't
                                                                * get logs */
        ],
        &config,
    );
...
fn test_commit() {
    ...
    for (i, (matches, logs, sm_term, w)) in tests.drain(..).enumerate() {
        ...
        store.wl().set_hard_state(HardState {
            term: sm_term,
            ..Default::default()
        });
...
// 2. If an existing entry conflicts with a new one (same index but different terms), delete the
//    existing entry and all that follow it; append any new entries not already in the log.
...
fn test_read_only_option_safe() {
    ...
    for (i, (id, proposals, wri, wctx, pending)) in tests.drain(..).enumerate() {
        ...
        // `pending` indicates that a `ReadIndex` request will not get through quorum checking
        // immediately so that it remains in the `read_index_queue`
        if pending {
            // drop MsgHeartbeatResponse here to prevent leader handling pending ReadIndex request
            // per round
            nt.ignore(MessageType::MsgHeartbeatResponse);
            nt.send(vec![msg1.clone(), msg1.clone(), msg2.clone()]);
            nt.recover();
            // send a ReadIndex request with the last ctx to notify leader to handle pending read
            // requests
...
fn test_read_only_for_new_leader() {
    ...
    for (id, committed, applied, compact_index) in node_configs {
        ...
        storage.wl().set_hard_state(HardState {
            term: 1,
            commit: committed,
            ..Default::default()
        });
...
fn test_step_ignore_config() {
    ...
    let mut m = new_message(1, 1, MessageType::MsgPropose, 0);
    let c = ConfChange {
        changes: vec![new_conf_change_single(1, ConfChangeType::AddNode)],
        ..Default::default()
    };
    let mut e = Entry::default();
    e.set_entry_type(EntryType::EntryConfChange);
    e.data = c.encode_to_vec();
    m.entries.push(e);
    assert!(!r.has_pending_conf());
...
fn test_commit_after_remove_node() -> Result<()> {
    ...
    let cc = ConfChange {
        changes: vec![new_conf_change_single(2, ConfChangeType::RemoveNode)],
        ..Default::default()
    };
    entry.data = cc.encode_to_vec();
...
fn test_restore_learner() {
    ...
    let mut sm = new_test_raft(3, vec![], 10, 1, new_storage());
...
fn test_learner_receive_snapshot() {
    ...
    let msg = Message {
        from: 1,
        to: 1,
        msg_type: MessageType::MsgBeat as i32,
        ..Default::default()
    };
    network.send(vec![msg]);
...
fn test_conf_change_check_before_campaign() {
    ...
    e.set_entry_type(EntryType::EntryConfChange);
    let cc = ConfChange {
        changes: vec![new_conf_change_single(3, ConfChangeType::RemoveNode)],
        ..Default::default()
    };
...
fn test_advance_commit_index_by_vote_request(use_prevote: bool) {
    let mut cases = vec![
        conf_change_with_single(ConfChangeType::AddNode, 4),
        conf_change(vec![
    ...
    for (i, cc) in cases.drain(..).enumerate() {
        ...
        p2.apply_conf_change(&cc).unwrap();
        ...
        p4.apply_conf_change(&cc).unwrap();
...
fn test_advance_commit_index_by_vote_response(use_prevote: bool) {
    let mut cases = vec![
        conf_change_with_single(ConfChangeType::RemoveNode, 4),
        // Explicit leave joint
        conf_change(vec![]),
    ];
    // Enter joint confchange
    let mut enter_joint = conf_change(vec![
...
fn prepare_request_snapshot() -> (Network, Snapshot) {
    fn index_term_11(id: u64, ids: Vec<u64>) -> Interface {
        ...
        store
            ...
            .unwrap();
        let mut raft = new_test_raft(id, ids, 5, 1, store);
        raft.reset(11);
        raft
    }
...
fn test_election_with_priority_log() {
    ...
    for (_i, &(l1, l2, l3, p1, p2, p3, id, state)) in tests.iter().enumerate() {
        ...
        let mut network = Network::new(vec![Some(n1), Some(n2), Some(n3)]);
...
fn test_read_when_quorum_becomes_less() {
    ...
    let m = Message {
        to: 1,
        msg_type: MessageType::MsgReadIndex as i32,
        entries: vec![Entry {
            data: (b"abcdefg" as &'static [u8]).into(),
            ..Default::default()
        }],
        ..Default::default()
    };
...
fn test_uncommitted_entries_size_limit() {
    ...
    let mut nt = Network::new_with_config(vec![None, None, None], config);
    let entry = Entry {
        data: (b"hello world!" as &'static [u8]).into(),
        ..Default::default()
    };
    ...
    let entry = Entry {
        data: (b"hello world and raft" as &'static [u8]).into(),
        ..Default::default()
    };
...
fn test_uncommitted_entry_after_leader_election() {
    ...
    let mut nt = Network::new_with_config(vec![None, None, None, None, None], config);
    let entry = Entry {
        data: (b"hello world!" as &'static [u8]).into(),
        ..Default::default()
    };
    ...
    assert_eq!(nt.peers.get_mut(&2).unwrap().state, StateRole::Leader);
...
fn test_fast_log_rejection() {
    ...
    ) in tests.drain(..).enumerate()
    {
        ...
        let mut n1 = new_test_raft(1, vec![1, 2, 3], 10, 1, s1);
        let mut n2 = new_test_raft(2, vec![1, 2, 3], 10, 1, s2);
        n1.become_candidate();
        n1.become_leader();
